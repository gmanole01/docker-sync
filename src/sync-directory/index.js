...
const formatParams = (srcDir, targetDir, customOptions = {}) => {
  ...
  if (!fs.existsSync(srcDir)) {
    throw new Error(
      `[sync-directory] "srcDir" folder does not exist: "${srcDir}"`
    );
  }
  ...
  options.include =
    options.include === null
      ? () => true
      : matchUtil.toFunction(options.include);
  // priot: 2
  options.exclude =
    options.exclude === null
      ? () => false
      : matchUtil.toFunction(options.exclude);
  // priot: 3
  options.forceSync =
    options.forceSync === null
      ? () => false
      : matchUtil.toFunction(options.forceSync);
...
const synced = (...args) => {
  ...
  if (params) {
    ...
    if (!options.skipInitialSync) {
      if (options.beforeInitialSync) {
        options.beforeInitialSync();
      }
      syncLocalFiles.sync(srcDir, targetDir, options);
      if (options.afterInitialSync) {
        options.afterInitialSync();
      }
    }
