...
const createTargetHardlinkFiles = ({
  relativeFilePath,
  srcPath,
  targetPath,
  staySymlink,
  ...
  try {
    ...
    if (staySymlink && isSymlink) {
      // if targetPath is a symlink dir
      fse.ensureSymlinkSync(srcPath, targetPath);
      ignoredSymlinkDirs.push(srcPath);
    } else {
      if (stats.isFile()) {
        // if targetPath is file
        if (fs.existsSync(targetPath)) {
          // recreate a new hardlink
...
const syncProcessor = (
  ...
  const srcFiles = readdirSync(
    srcDir,
    nodeep,
    filter(srcDir, exclude, forceSync)
  );

  const targetFiles = readdirSync(
    targetDir,
    nodeep,
    filter(targetDir, exclude, forceSync)
  );
...
const asyncProcessor = async (
  ...
  const srcFiles = await readdirAsync(
    srcDir,
    nodeep,
    filter(srcDir, exclude, forceSync)
  );

  const targetFiles = await readdirAsync(
    targetDir,
    nodeep,
    filter(targetDir, exclude, forceSync)
  );
